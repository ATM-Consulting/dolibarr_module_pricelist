...
llxHeader('', $langs->trans('pricelistList'), '', '');

//$type = GETPOST('type');
//if (empty($user->rights->pricelist->all->read)) $type = 'mine';
...
$sql.= ' FROM '.MAIN_DB_PREFIX.'pricelist t ';

if (!empty($object->isextrafieldmanaged))
{
    $sql.= ' LEFT JOIN '.MAIN_DB_PREFIX.'pricelist_extrafields et ON (et.fk_object = t.rowid)';
}

$sql.= ' WHERE 1=1';
//$sql.= ' AND t.entity IN ('.getEntity('pricelist', 1).')';
...
$formcore = new TFormCore($_SERVER['PHP_SELF'], 'form_list_pricelist', 'GET');

$nbLine = !empty($user->conf->MAIN_SIZE_LISTE_LIMIT) ? $user->conf->MAIN_SIZE_LISTE_LIMIT : $conf->global->MAIN_SIZE_LISTE_LIMIT;

$r = new Listview($db, 'pricelist');
echo $r->render($sql, array(
    ...
    ,'list' => array(
        'title' => $langs->trans('pricelistList')
